...
import { debounceTime, delay, map, Observable, Subject, switchMap, tap, throttleTime } from 'rxjs';
...
export class WalletService {
  ...
  constructor(@Inject(DOCUMENT) document: Document) {
    ...
    this.signer$ = this.accountsChange$.pipe(map(() => this.provider.getSigner()));
    this.address$ = this.signer$.pipe(switchMap((signer) => signer.getAddress().catch(() => undefined)));
    ...
    window.ethereum.on("accountsChanged", () => this.accountsChange$.next(null));
  }
